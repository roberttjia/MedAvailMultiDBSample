...
namespace MedAvail.DataAccess.Ef6
{
    ...
    /// connection-string provider. Each context gets its own NpgsqlConnection,
    /// which it owns and disposes. This is the single point binding EF6 to a
    /// database.
    /// </summary>
    public sealed class Ef6ContextFactory
    {
        ...
        public CoreEf6Context CreateCore()
        {
            var cs = _connections.GetConnectionString(MedAvailDatabase.Core);
            var connection = new NpgsqlConnection(cs);
            return new CoreEf6Context(connection, contextOwnsConnection: true);
        }
    }
}
