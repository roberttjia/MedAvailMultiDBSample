...
RETURNING package_definition_id;";
            ...
            cmd.Parameters.Add(Param("@description", NpgsqlDbType.Varchar, definition.Description));
            cmd.Parameters.Add(Param("@package_code", NpgsqlDbType.Varchar, definition.PackageCode));
            ...
            cmd.Parameters.Add(Param("@product_name", NpgsqlDbType.Varchar, definition.ProductName));
            cmd.Parameters.Add(Param("@prodcodetype", NpgsqlDbType.Integer, definition.ProductCodeTypeId));
            cmd.Parameters.Add(Param("@product_code", NpgsqlDbType.Varchar, definition.ProductCode));
            cmd.Parameters.Add(Param("@manufacturer", NpgsqlDbType.Varchar, definition.ProductManufacturer));
            ...
            cmd.Parameters.Add(Param("@changed_by", NpgsqlDbType.Varchar, definition.ChangedBy));
            cmd.Parameters.Add(Param("@controlled", NpgsqlDbType.Numeric, definition.ControlledSubstance));
            cmd.Parameters.Add(Param("@created_by", NpgsqlDbType.Varchar, definition.CreatedBy));
            ...
            cmd.Parameters.Add(Param("@isdemo", NpgsqlDbType.Numeric, definition.IsDemoPackage));
        ...
        /// <summary>
        /// Reads the auditid_ma (BIGINT) value for a row.
        /// Returns the value as a long; the caller may convert to byte[] for legacy compatibility.
        /// </summary>
        public long GetRowVersionLong(int packageDefinitionId)
        {
            using var conn = OpenConnection();
            using var cmd = conn.CreateCommand();
        ...
        /// <summary>
        /// Reads the auditid_ma value and returns it as a byte[] for interface compatibility.
        /// </summary>
        public byte[]? GetRowVersion(int packageDefinitionId)
        {
            var val = GetRowVersionLong(packageDefinitionId);
            return BitConverter.GetBytes(val);
        }

        /// <summary>
        /// Sets the cap/valid (bool) and controlled_substance/is_demo_package (decimal/NUMERIC)
        /// columns for a row, returning rows affected.
        /// </summary>
        public int UpdateBitFlags(int packageDefinitionId, bool cap, bool valid,
            decimal controlledSubstance, decimal isDemo)
        {
            using var conn = OpenConnection();
            using var cmd = conn.CreateCommand();
            cmd.CommandText =
            ...
            cmd.Parameters.Add(Param("@cs", NpgsqlDbType.Numeric, controlledSubstance));
            cmd.Parameters.Add(Param("@demo", NpgsqlDbType.Numeric, isDemo));
            cmd.Parameters.Add(Param("@id", NpgsqlDbType.Integer, packageDefinitionId));
            return cmd.ExecuteNonQuery();
        }
        ...
        /// auditid_ma BIGINT. Returns rows affected (0 means the token was stale).
        /// </summary>
        public int UpdateDescriptionIfVersionMatches(int packageDefinitionId, string description, long expectedRowVersion)
        {
            using var conn = OpenConnection();
            using var cmd = conn.CreateCommand();
            cmd.CommandText =
            ...
            cmd.Parameters.Add(Param("@desc", NpgsqlDbType.Varchar, description));
            cmd.Parameters.Add(Param("@id", NpgsqlDbType.Integer, packageDefinitionId));
            cmd.Parameters.Add(Param("@rv", NpgsqlDbType.Bigint, expectedRowVersion));
            return cmd.ExecuteNonQuery();
        }
        ...
        private static PackageDefinitionDto Map(NpgsqlDataReader r)
        {
            return new PackageDefinitionDto
            {
                ...
                Valid = Convert.ToDecimal(r[17]) != 0,
