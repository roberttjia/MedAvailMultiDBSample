...
public sealed class AdoTests
{
    ...
    // ---- cross-database access (PostgreSQL single-database model) ----
    // Proves the local package_id_map query works and the removemedcenter proc path executes.
    private void CrossDatabase(TestRunnerHarness harness)
    {
        ...
        harness.Run("Local package_id_map query (Core schema)", cat, "Ado", () =>
        {
            // Connected to MedAvailDB, query the local package_id_map table.
            var count = repo.CountPackageIdMapInPackageManagement();
            Assert.True(count >= 0, "local package_id_map query should resolve and return a count");
        });
        ...
        harness.Run("Cross-database stored proc RemoveMedCenter (rolled back)", cat, "Ado", () =>
        {
            ...
            // always rolls back. Exercises the REFCURSOR proc path safely.
            var serial = AdoCrossDatabaseRepository.NewNonExistentSerial();
            var ok = repo.InvokeRemoveMedCenterInRollback(serial);
            Assert.True(ok, "RemoveMedCenter should run without error");
        });
    }

    // ---- auditid_ma (BIGINT) + numeric field behavior (PackageManagement) ----
    // Seeds + inserts a dedicated row, then exercises auditid_ma and numeric columns.
    private void RowVersionAndBitFields(TestRunnerHarness harness)
    {
        ...
        harness.Run("auditid_ma is populated after insert", cat, "Ado", () =>
        {
            Assert.True(id > 0, "insert should have produced an id");
            auditIdAfterInsert = repo.GetRowVersionLong(id);
            Assert.True(auditIdAfterInsert > 0, "auditid_ma should be a positive BIGINT generated by the sequence");
        });

        harness.Run("auditid_ma changes after update", cat, "Ado", () =>
        {
            Assert.True(auditIdAfterInsert > 0, "need the initial auditid_ma");
            // Any update bumps the auditid_ma via the trigger.
            repo.UpdateBitFlags(id, cap: true, valid: false, controlledSubstance: 1m, isDemo: 0m);
            var after = repo.GetRowVersionLong(id);
            Assert.True(after > 0, "auditid_ma should still be present");
            Assert.True(after != auditIdAfterInsert, "auditid_ma must change after an update");
        });

        harness.Run("numeric fields round-trip", cat, "Ado", () =>
        {
            ...
            Assert.Equal(true, row!.Cap, "cap should be true");
            Assert.Equal(false, row.Valid, "valid should be false");
            Assert.Equal(1m, row.ControlledSubstance, "controlled_substance should be 1");
            Assert.Equal(0m, row.IsDemoPackage, "is_demo_package should be 0");

            // Flip them all and confirm the inverse round-trips too.
            repo.UpdateBitFlags(id, cap: false, valid: true, controlledSubstance: 0m, isDemo: 1m);
            ...
            Assert.Equal(0m, flipped.ControlledSubstance, "controlled_substance should flip to 0");
            Assert.Equal(1m, flipped.IsDemoPackage, "is_demo_package should flip to 1");
        });

        harness.Run("optimistic concurrency via auditid_ma", cat, "Ado", () =>
        {
            Assert.True(id > 0, "need an inserted row");
            var current = repo.GetRowVersionLong(id);

            // A matching token should update exactly one row.
            var matched = repo.UpdateDescriptionIfVersionMatches(id, "rv-match", current);
            Assert.Equal(1, matched, "update with current auditid_ma should succeed");

            // Reusing the now-stale token should affect zero rows.
            var stale = repo.UpdateDescriptionIfVersionMatches(id, "rv-stale", current);
            Assert.Equal(0, stale, "update with stale auditid_ma should affect no rows");
        });
    }
